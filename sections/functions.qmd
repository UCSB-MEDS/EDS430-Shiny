...
<p class="small-text">`r fontawesome::fa("box-open", fill = "#05859B", a11y = "sem")` `{tbd}`: tbd</p>
...
we not only have reduced 10 lines of UI code into 2, but we also have made our code a bit more readable -- rather than a collaborator walking through our code and having to interpret what those two pickerInputs were doing using code comments and arguments, they instead can now see from our function name that those lines of code are likely pickerInputs that allow users to select penguin species.

---

Don't have to only write functions for duplicated UI elements -- can also turn elements into fucntions with more descriptive names so that your code is easier to read
