...
<p class="small-text">To start, let's add our server function to `server.R` and the three main UI components (header, sidebar, and body) to our dashboard page. You can do so just as the example on [slide 110](), but alternatively, you can split the UI into separate pieces, then combine them into a `dashboardPage` the end of `ui.R` -- this can help with organization as you app grows in complexity.</p>
...
Add `title` to `dashboardHeader()`

Add `sidebarMenu` to `dashboardbody()` -- sidebarMenu behaves similarly to tabs in shiny app `tabPanel` but it's also more flexible and can contain shiny inputs

Add `menuItem`s to `sidebarMenu`
