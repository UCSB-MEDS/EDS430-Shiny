...
<div class="slide-title-smaller">Designing our dashboard UI</div>

<p class="small-text">Now that we have a better sense of what our app will include, let's start building out our UI.</p>
...
<div class="slide-title-smaller">Add a title & menuItems</div>

<p class="small-text">Add a title to `dashboardHeader()` and make more space using `titleWidth`, if necessary.</p>

<p class="small-text">Next, we'll create a `sidebarMenu()` that contains two `menuItem`s. `sidebarMenu()` can also contain shiny inputs (read more [here](https://rstudio.github.io/shinydashboard/structure.html#sidebar)), though we'll be using ours just for navigation purposes. Be sure to provide each `menuItem()` with `text` as you'd like it to appear in your app, and a `tabName` which will be used to place `dashboardBody()` content in the appropriate `menuItem()`. Optionally, you can provide an icon. By default, `icon()` uses icons from [FontAwesome](https://fontawesome.com/).</p>
...
##  {#fluidRow data-menu-title="fluidRow()"}

<div class="slide-title-smaller">Adding `tabPanel`s to boxes</div>

<p class="small-text">Lastly, we'll add two side-by-side boxes to our **welcome** tab: one normal `box` and one `tabBox`. Use `fluidRow()` to place both boxes in the same row, with each taking up half the page. Create two `tabPanel`s within the `tabBox` -- we'll eventually place field site photos in each `tabPanel`.</p>
...
##  {#add-leaflet-placeholder data-menu-title="Add Leaflet Map & Widgets"}

<div class="slide-title-smaller">Add a `sliderInput` & `leafletOutput` to the UI</div>

<p class="small-text">I'll start by adding just one `sliderInput` to select a range of `Elevation`s to filter our map makers by. I'm also adding a `leafletOutput` to create a placeholder space for our map, along with a Spinner (from the `{shinycssloaders}` package). My inputs and map output will both live inside the same `box`.</p>

```{r}
#| eval: false
#| echo: true
#| code-line-numbers: "78-84"
........................dashboardHeader.........................
header <- dashboardHeader(
  ...
  tabItems(
    ...
    tabItem(tabName = "welcome",
            ...
            fluidRow(
              ...
              tabBox(width = 6, 
                     ...
                     title = tagList(shiny::icon("camera"), "Photos from Fish Creek"),
            ...
            ) # END intro information fluidRow
    ...
    tabItem(tabName = "dashboard",
            ...
            box(width = 12,
                ...
                # sliderInputs ----
                sliderInput(inputId = "elevation_slider", label = "Select and Elevation range (meters above SL):",
...
<div class="slide-title-smaller">Assemble your `sliderInput` and `leafletOutput` in `server.R`</div>
...
<center><img src="images/elev_widget.gif" alt="Our Fish Creek Watershed Lake Monitoring dashboard. The 'Welcome' page has two side-by-side boxes, the left one with placeholder text reading 'background info here' and the right one with two tabs and the title 'Photos from Fish Creek'. The 'Dashboard' tab has a sliderInput to select a range of elevations. When a range is selected, the blue markers on the map are filtered accordingly. Clicking on the markers reveals more information about the specific site, including Site Name, Elevation, Avg Depth, and AvgTemp." width="750"/></center>

---

##  {#exercise5-prompt data-menu-title="Exercise 5 Prompt"}

<div class="slide-title-smaller">`r fontawesome::fa("question", fill = "#05859B", a11y = "sem")` `r fontawesome::fa("lightbulb", fill = "#05859B", a11y = "sem")`Exercise 5: Add two more `sliderInput`s to filter for `AvgDepth` & `AvgTemp`</div>

<p class="body-text">**To Do:**

<p class="small-text topbr">`r fontawesome::fa("angle-right", fill = "#05859B", a11y = "sem")` Add two more `sliderInput`s, one for `AvgDepth` and one for `AvgTemp` beneath our first Elevation widget in the UI</p>
...
<p class="medium-body-text">See next slide for a solution!</p>
