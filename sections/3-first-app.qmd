...
##  {#build-app1 data-menu-title="Building Out Your 1st App" background="#053660"}
...
##  {#LO1 data-menu-title="Learning Objectives 1"}

<div class="slide-title">`r fontawesome::fa("book-open", fill = "#05859B", a11y = "sem")` Learning Objectives for App #1</div>

<p class="medium-body-text bottombr">By the end of building out this first app, you should be a bit more familiar with:</p>

. .  .

<p class="small-text">`r fontawesome::fa("angle-right", fill = "#05859B", a11y = "sem")` writing a single-file (`app.R`) shiny app</p>

. . . 

<p class="small-text">`r fontawesome::fa("angle-right", fill = "#05859B", a11y = "sem")` adding and styling text in the UI using `tags`</p>

. . . 

<p class="small-text">`r fontawesome::fa("angle-right", fill = "#05859B", a11y = "sem")` practicing data wrangling and visualization *outside* of your shiny app</p>

. . . 

<p class="small-text">`r fontawesome::fa("angle-right", fill = "#05859B", a11y = "sem")` following a general workflow for building reactive apps, which includes adding **inputs** and **outputs** to the **UI**, then writing the **server instructions** on how to assemble user input values into outputs</p>

. .  .

<p class="small-text">`r fontawesome::fa("angle-right", fill = "#05859B", a11y = "sem")` running and quitting apps in RStudio</p>

. . . 

<p class="medium-body-text topbr">Packages used:</p> 

. . . 

<p class="small-text">`r fontawesome::fa("box-open", fill = "#05859B", a11y = "sem")` `{shiny}`: framework for building our reactive app + standard widgets</p>

. . . 

<p class="small-text">`r fontawesome::fa("box-open", fill = "#05859B", a11y = "sem")` `{DT}`: interactive datatable widgets (that can be made reactive using shiny!)</p>

. . . 

<p class="small-text">`r fontawesome::fa("box-open", fill = "#05859B", a11y = "sem")` `{tidyverse}`: collection of packages for wrangling & visualizing data</p>   

. . . 

<p class="small-text">`r fontawesome::fa("box-open", fill = "#05859B", a11y = "sem")` `{palmerpenguins}`: data</p>
...
<p class="smaller-text">**I sometimes like to think to myself:** "First, I'm going to tell my server to create an output using the syntax `output$`. Then, I will tell my server to place that output in the placeholder named "bodyMass_scatterPlot" (created in my UI) using the syntax `output$bodyMass_scatterPlot`. </p>
...
<div class="slide-title-smaller">**An Aside:** Draft objects (e.g. plots) in a separate script first</div>
...
<p class="small-text important-text-bg">**Tip:** Save your practice script in a separate directory (i.e. not inside your app directory) -- I typically save mine to something like `~/R/practice_script.R`.</p>

---

##  {#rule2.3 data-menu-title="Copy code into server"}

<div class="slide-title-smaller">Copy your plot code into the server</div>
...
<div class="slide-title-smaller">A non-reactive plot now lives in our `plotOutput()` placeholder</div>
...
<div class="slide-title-smaller">Practice filtering data in our separate script</div>
...
<div class="slide-title-smaller">Which part of our code needs to be updated when a user changes the slider range input?</div>
...
<p class="small-text">Recall that in our **UI**, we gave our `sliderInput()` an `inputId = "body_mass"`.</p>
...
<p class="smaller-text">In our server, we can access the values of that slider input using the syntax, `input$body_mass`. *If you want your output to change according to the input values, substitute hard-coded values (e.g. 3725:5191) with the input values from the UI (e.g. `input$body_mass[1]:input$body_mass[2]`)*.</p>
...
<div class="slide-title">Okay, RUN THAT APP!</div>
...
<div class="slide-title-smaller">Recap: We created our first *reactive* Shiny app following these steps: </div>

. . . 

<p class="body-text">**1.** We created an `app.R` file in it's own directory and began our app with the [template](http://localhost:7907/#/single-file), though you can also create a [two-file Shiny app](http://localhost:7907/#/two-file) by using separate `ui.R` and `server.R` files.</p>
...
<p class="small-text">`r fontawesome::fa("angle-right", fill = "#05859B", a11y = "sem")` save objects that you want to display to `output$<id>`</p>
<p class="small-text">`r fontawesome::fa("angle-right", fill = "#05859B", a11y = "sem")` build reactive objects using a `render*()` function (and similarly, build reactive data frames using `reactive()`</p>
<p class="small-text">`r fontawesome::fa("angle-right", fill = "#05859B", a11y = "sem")` access input values with `input$<id>`</p>

. . . 

<center><p class="medium-body-text topbr">And we saw that reactivity automatically occurs whenever we use an ***input value*** to ***render*** an ***output object***.</p></center>

---

##  {#app1-ex1 data-menu-title="Exercise 1 Prompt"}

<div class="slide-title-smaller">`r fontawesome::fa("question", fill = "#05859B", a11y = "sem")` Exercise 1: Add another reactive widget</div>

<p class="small-text bottombr">`{DT}` is a package that provides an [R interface](https://rstudio.github.io/DT/) to the JavaScript library [DataTables](https://datatables.net/) (you may have already used the DT package in your knitted RMarkdown/Quarto HTML documents). DT datatables allow for filtering, pagination, sorting, and lots of other neat features for tables on your HTML pages.</p>
...
<div class="slide-title-smaller">`r fontawesome::fa("lightbulb", fill = "#05859B", a11y = "sem")` Exercise 1: Tips</div>

<p class="body-text topbr">**Tips:**</p>
<p class="smaller-text">`r fontawesome::fa("angle-right", fill = "#05859B", a11y = "sem")` Use `?checkboxGroupInput` to learn more about which arguments you need (remember, all inputs require an `inputId` and oftentimes a `label`, but there are others required to make this work as well)</p>
<p class="smaller-text">`r fontawesome::fa("angle-right", fill = "#05859B", a11y = "sem")` Both `{shiny}` and `{DT}` packages have functions named `dataTableOutput()` and `renderDataTable()`   (`DT::renderDataTable()` allows you to create both server-side and client-side DataTables and supports additional DataTables features while `shiny::renderDataTable()` only provides server-side DataTables). Be sure to use the one from the `{DT}` package using the syntax `packagename::functionname()`.</p>
<p class="smaller-text bottombr">`r fontawesome::fa("angle-right", fill = "#05859B", a11y = "sem")` There are lots of ways to customize DT tables, but to create a basic one, all you need is to `DT::dataTable(your_dataframe)`<p>


<p class="body-text topbr">**And remember** to follow the steps outlined on the previous slides (jump back to [slide 26](https://ucsb-meds.github.io/EDS430-Shiny/#/reactive-plot-app1)):</p>
...
<div class="slide-title-smaller">`r fontawesome::fa("check", fill = "#05859B", a11y = "sem")` Exercise 1: A solution</div>
...
ui <- fluidPage(
  ...
  # title & subtitle (omitted for brevity) ----
  
  # body mass slider input & output (omitted for brevity) ----

  # year input ----
  checkboxGroupInput(inputId = "year", label = "Select year(s):",
                     choices = c("The year 2007" = "2007", "2008", "2009"), # OR `unique(penguins$year)` | NOTE: update display name using "New Name" = "observation name"
...
server <- function(input, output) {
  ...
  # reactive scatterplot (omitted for brevity) ----
  
  # filter for years ----
  years_df <- reactive({
    penguins |> 
      filter(year %in% input$year) # `%in%` = return observations where year is "in" the set of options provided by the user via the checkboxGroupInput
  })
  
  # render the DT::datatable ----
  output$penguin_data <- DT::renderDataTable({
    
    DT::datatable(years_df(), # check out https://rstudio.github.io/DT/ for lots of fun ways to customize your table!
...
<center><p class="medium-body-text topbr">Find the complete code for app #1 on [GitHub `r fontawesome::fa("github", fill = "#05859B", a11y = "sem")` ](https://github.com/samanthacsik/EDS430-shiny-app/blob/main/one-file-app/app.R).</p></center>

---

##  {#common-mistakes data-menu-title="Common Mistakes"}

<div class="slide-title-smaller">Common mistakes to look out for</div>

<p class="body-text">It's inevitable that you'll make mistakes here and there as you build out your app...and they can be frustrating to catch. The three that I find myself making over and over again are:</p>
...
::: {.column width="45%"}
<p class="small-text">`r fontawesome::fa("angle-right", fill = "#05859B", a11y = "sem")` misspelling `inputId` (or `outputId`) with a capital `ID` rather than `Id`</p>

<p class="small-text topbr">`r fontawesome::fa("angle-right", fill = "#05859B", a11y = "sem")` misspelling your inputId (or outputId) in the server (e.g. UI: `inputId = "myInputID"`, server: `input$my_Input_ID`)</p>

<p class="small-text topbr">`r fontawesome::fa("angle-right", fill = "#05859B", a11y = "sem")` forgetting to separate UI elements with a comma, `,`</p>

<p class="small-text topbr">`r fontawesome::fa("angle-right", fill = "#05859B", a11y = "sem")` forgetting the set of parentheses when calling the name of a reactive data frame in a plot (e.g. `ggplot(my_reactive_df(), aes(...))`)
:::
...
##  {#app1-recap data-menu-title="App 1 Recap"}

<div class="slide-title-smaller">`r fontawesome::fa("check", fill = "#05859B", a11y = "sem")` So far, we've talked about:</div>

. . . 

<p class="small-text topbr">`r fontawesome::fa("angle-right", fill = "#05859B", a11y = "sem")` how to find working **examples** of apps (built-in using the `{shiny}` package and via other online resources)</p>

. . . 

<p class="small-text topbr">`r fontawesome::fa("angle-right", fill = "#05859B", a11y = "sem")` the **anatomy of a shiny app** (UI & server) and **how to set one up** (both single and two-file apps -- though more on two-file apps soon!)</p>

. . .

<p class="small-text topbr">`r fontawesome::fa("angle-right", fill = "#05859B", a11y = "sem")` **running** and **quitting** a shiny app (and opening it up in your browser window)</p>

. . .

<p class="small-text topbr">`r fontawesome::fa("angle-right", fill = "#05859B", a11y = "sem")` adding and styling **text** using `tags`</p>

. . .

<p class="small-text bottombr topbr">`r fontawesome::fa("angle-right", fill = "#05859B", a11y = "sem")` **reactivity** (using input values to render outputs objects) and a **general workflow** to follow to create reactive outputs -- which includes the following:</p>

. . .

<p class="smaller-text topbr">`r fontawesome::fa("caret-right", fill = "#05859B", a11y = "sem")` implementing **widgets** from the `{shiny}` package using `*Input()` functions (specifically `sliderInput()` & `checkboxGroupInput()`)</p>

. . .

<p class="smaller-text topbr">`r fontawesome::fa("caret-right", fill = "#05859B", a11y = "sem")` creating **placeholders** in the UI for our reactive outputs to live using `*Output()` functions (specifically `plotOutput()` and `DT::datatableOutput()`)</p>

. . .

<p class="smaller-text topbr">`r fontawesome::fa("caret-right", fill = "#05859B", a11y = "sem")` writing the server instructions for **rendering outputs** using `render*()` functions (specifically `renderPlot()` & `DT::renderDataTable()`)</p>

. . .

<p class="smaller-text topbr">`r fontawesome::fa("caret-right", fill = "#05859B", a11y = "sem")` creating **reactive data frames** using `reactive()`</p>

. . .

<p class="small-text topbr">`r fontawesome::fa("angle-right", fill = "#05859B", a11y = "sem")` Importantly, **practice building outputs in a separate script** before trying to make them reactive!</p>
